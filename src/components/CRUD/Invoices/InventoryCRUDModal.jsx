...
import { updateInvoices } from "../../../redux/reducers/invoice";
  ...
  const onEditInventory = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setShowModal(false);
        dispatch(updateInvoices(invoices.map((inv) => inv._id === inventory._id ? response.data.inventory : inv)));
      };
    } catch (error) {
      console.error(error);
    }
  ...
  const onDeleteInventory = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setShowModal(false);
        dispatch(updateInvoices(invoices.filter((inv) => inv._id !== inventory._id)));
      }
    } catch (error) {
      console.error(error);
    }
        ...
        >
          ...
          >
            ...
            >
              ...
              <Text style={{ marginLeft: 10, fontSize: 12 }}>{inventory.description}</Text>
            </Box>
          </Box>
        </Box>
      </Pressable>
