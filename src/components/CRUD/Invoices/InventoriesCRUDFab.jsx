...
export default function InventoriesCRUDFab() {
  ...
  useEffect(() => {
    let shoudDisable = get(newInventory, "name", "").length === 0 || get(newInventory, "price", "").length === 0 || get(newInventory, "description", "").length === 0;
  ...
  const onAddNewInventory = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        dispatch(updateInvoices(invoices.concat(response.data.inventory)));
        setShowModal(false);
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <Box>
      ...
      >
        ...
        <ModalContent>
          <ModalHeader>
            <Heading size='lg'>Add Invoice</Heading>
